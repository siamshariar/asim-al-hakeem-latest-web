...
const loadQnaData = () => {
	if (!qnaData) {
		...
		// Try public data first (Vercel), then local data directory
		const possibleDirs = [
			path.join(process.cwd(), 'public', 'data', 'qna'),
			path.join(process.cwd(), 'data', 'qna'),
		];

		let qnaDirFound = null;
		try {
			for (const p of possibleDirs) {
				if (fs && fs.existsSync && fs.existsSync(p) && fs.statSync(p).isDirectory()) {
					qnaDirFound = p;
					break;
				}
			}
			    ...
			    if (!qnaDirFound) {
					console.error('Cannot find qna data directory. Tried:', possibleDirs);
					// Log parent directory contents where possible to aid Vercel debugging
					...
					const singlePath = path.join(process.cwd(), 'data', 'qna.json');
					if (fs.existsSync(singlePath)) {
						qnaData = JSON.parse(fs.readFileSync(singlePath, 'utf8'));
					} else {
						qnaData = [];
					}
				} else {
				const files = fs.readdirSync(qnaDirFound).filter((f) => f.startsWith('qna-') && f.endsWith('.json')).sort();
				let all = [];
				files.forEach((f) => {
					try {
						const raw = fs.readFileSync(path.join(qnaDirFound, f), 'utf8');
						const parsed = raw ? JSON.parse(raw) : [];
						if (Array.isArray(parsed)) all = all.concat(parsed);
					} catch (err) {
						console.error('Error reading qna chunk', f, err && err.message);
					}
				});
				qnaData = all;
			}

			// Load category data from data or public/data
			const qnCatCandidates = [
				path.join(process.cwd(), 'data', 'qnCat.json'),
				path.join(process.cwd(), 'public', 'data', 'qnCat.json'),
			];
			let qnCatPath = null;
			for (const p of qnCatCandidates) {
				if (fs.existsSync(p)) { qnCatPath = p; break; }
			}
			if (qnCatPath) {
			...
			console.error('Error reading qna json', e && e.message);
			qnaData = [];
			qnCatData = [];
		}
	}
...
// Attempt to fetch public JSON chunks over HTTP when filesystem access fails (useful on some Vercel runtimes)
const fetchQnaFromPublicOrigin = async () => {
	if (typeof window !== 'undefined') return { qna: [], qnCat: [] };

	// Determine origin
	const origin = process.env.NEXT_PUBLIC_SITE_URL
		|| (process.env.VERCEL_URL ? `https://${process.env.VERCEL_URL}` : null)
		|| 'https://asim-al-hakeem-latest-web-view.vercel.app';
	...
	try {
		// fetch qnCat.json
		try {
			const catRes = await fetch(`${origin}/data/qnCat.json`);
			if (catRes.ok) {
				fetchedCat = await catRes.json();
			}
		} catch (e) {}

		// Try sequential chunk names qna-000.json ... qna-020.json
		for (let i = 0; i < 25; i++) {
			const name = `qna-${String(i).padStart(3, '0')}.json`;
			try {
				const res = await fetch(`${origin}/data/qna/${name}`);
				if (!res.ok) {
					// stop if 404 (assume no more chunks)
	...
	return { qna: fetched, qnCat: fetchedCat };
};

const ensureQnaLoaded = async () => {
	if (qnaData && Array.isArray(qnaData) && qnaData.length) return;
	// Try filesystem first (sync)
	const local = loadQnaData();
	if (local.qna && local.qna.length) return;

	// Filesystem empty — try fetching public JSON over HTTP
	try {
		const remote = await fetchQnaFromPublicOrigin();
		if (remote.qna && remote.qna.length) {
			qnaData = remote.qna;
		}
		if (remote.qnCat && remote.qnCat.length) {
			qnCatData = remote.qnCat;
		}
	} catch (e) {
		// ignore
	}
...
export const getHomeQna = async () => {
	const { qna: rawQna } = loadQnaData();
	const normalizedItems = (rawQna || [])
		.map(normalizeQna)
...
export const getQnaByLimit = async (num) => {
	await ensureQnaLoaded();
	if (num && num > 0) {
		return (qnaData || []).slice(0, num).map(normalizeQna).filter(Boolean);
	} else return (qnaData || []).slice(0, constants.GENERATED_ANS_PAGE).map(normalizeQna).filter(Boolean);
};
...
export const getAnsById = async (id) => {
	await ensureQnaLoaded();
	return (qnaData || []).filter((item) => item.id == id).map(normalizeQna).filter(Boolean);
};

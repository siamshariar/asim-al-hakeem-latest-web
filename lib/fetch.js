...
const loadQnaData = () => {
    if (!qnaData) {
		...
		const qnaDir = path.join(process.cwd(), 'data', 'qna');

		try {
			// If a directory of chunked qna files exists, read and concat them in filename order.
			if (fs.existsSync(qnaDir) && fs.statSync(qnaDir).isDirectory()) {
				const files = fs.readdirSync(qnaDir).filter((f) => f.endsWith('.json')).sort();
				let all = [];
				files.forEach((f) => {
					try {
						const raw = fs.readFileSync(path.join(qnaDir, f), 'utf8');
				...
				qnaData = all;
			} else if (fs.existsSync(qnaPath)) {
				qnaData = JSON.parse(fs.readFileSync(qnaPath, 'utf8'));
			} else {
				qnaData = [];
			}

			if (fs.existsSync(qnCatPath)) {
